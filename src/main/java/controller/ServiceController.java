...
public class ServiceController {
    ...
    @PostMapping(value="/query/{type}", produces={"application/json;charset=UTF-8"})
    public String getWeekCourse(@PathVariable String type, @RequestParam(required=false, defaultValue="-1") int week, String openId, HttpServletRequest request) {
        JSONResult myJSON = new JSONResult();
        if ((week > 0 && type.equals("course")) || type.equals("grade")) { // 参数正确
            boolean flag = false;
            String auth = "";
            String account = null;
            ...
            myJSON.setStatusCode(400);
            myJSON.setMsg("parameters error");
        }
        return JSON.toJSONString(myJSON);
    }
}
